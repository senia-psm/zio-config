...
sealed trait ReadError[A] extends Exception { self =>
  override def toString: String = self match {
    ...
    case ReadError.ForceSeverity(error, treatAsMissing) => s"AndErrors(${error.toString},${treatAsMissing})"
  }
  ...
  def size: Int =
    self match {
      ...
      case ReadError.ForceSeverity(_, _)   => 1
    }
  ...
  final def prettyPrint(keyDelimiter: Char = '.'): String = {
    ...
    def renderForceSeverity[A](err: ReadError.ForceSeverity[A]): Sequential =
      Sequential(
        List(
          Failure(
            "ForceSeverity" :: s"cause: ${err.toString}" :: s"treatAsMissing: ${err.treatAsMissing}" :: Nil
          )
        )
      )
    ...
    def readErrorToSequential[A](readError: ReadError[A]): Sequential =
      readError match {
        ...
        case r: ReadError.ForceSeverity[A]   => renderForceSeverity(r)
